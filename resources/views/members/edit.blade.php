...
<div class="p-4 sm:p-6 lg:p-8">
    ...
    <div class="flex items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Modifier le membre</h1>
        ...
        <div class="hidden sm:flex items-center gap-2">
            ...
            @endphp
            <a href="{{ route('statistics.member', $member) }}" class="px-4 py-2 text-violet-600 rounded-lg font-semibold text-sm hover:bg-violet-50 transition-colors flex items-center gap-2">
            ...
            @if($currentRole && $currentRole->slug !== 'owner')
                <form action="{{ route('members.destroy', $member) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir retirer ce membre ?')" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 text-red-600 rounded-lg font-semibold text-sm hover:bg-red-50 transition-colors flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <form id="member-form" method="POST" action="{{ route('members.update', $member) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid lg:grid-cols-12 gap-6">
            ...
            <div class="lg:col-span-3 space-y-5">
                    ...
                    <div class="space-y-1.5">
                        @foreach($roles as $role)
                            <label class="relative flex items-center gap-2.5 p-2.5 rounded-xl border border-transparent cursor-pointer hover:bg-slate-50 transition-all has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50">
                                <input type="radio" name="role_id" value="{{ $role->id }}" data-slug="{{ $role->slug }}" data-color="{{ $role->color }}" class="sr-only peer role-radio" {{ old('role_id', $membership->pivot->role_id) == $role->id ? 'checked' : '' }}>
                    ...
                    <div class="space-y-3">
                        ...
                        <div id="field-jersey" class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input type="text" id="jersey_number" name="jersey_number" value="{{ old('jersey_number', $membership->pivot->jersey_number) }}"
                            ...
                            <div>
                                ...
                                <input type="date" id="joined_at" name="joined_at" value="{{ old('joined_at', $membership->pivot->joined_at ? \Carbon\Carbon::parse($membership->pivot->joined_at)->format('Y-m-d') : '') }}"
                        ...
                        <div id="field-position">
                            ...
                            <input type="text" id="position" name="position" value="{{ old('position', $membership->pivot->position) }}"
                        ...
                        <div id="field-license">
                            ...
                            <input type="text" id="license_number" name="license_number" value="{{ old('license_number', $membership->pivot->license_number) }}"
                                class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 focus:bg-white outline-none transition-all"
                ...
                <!-- Status -->
                <div class="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm">
                    <h2 class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Statut</h2>
                    <select name="status" id="status"
                        class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 focus:bg-white outline-none transition-all font-medium">
            ...
            <div class="lg:col-span-5 space-y-5">
                ...
                <div class="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        ...
                        placeholder="Informations complémentaires...">{{ old('notes', $membership->pivot->notes) }}</textarea>
                </div>
            </div>
            ...
            <div class="lg:col-span-4">
                <div class="lg:sticky lg:top-24 space-y-4">
                    ...
                    @php
                        $currentRole = \App\Models\Role::find($membership->pivot->role_id);
                        $currentColor = $currentRole->color ?? '#10B981';
                        $isPlayer = $currentRole && $currentRole->slug === 'player';
                    @endphp
                    
                    <!-- Preview Card - Dossard pour joueur -->
                    <div id="preview-player" class="{{ !$isPlayer ? 'hidden' : '' }} relative">
                        <!-- Maillot / Dossard -->
                        <div class="relative rounded-[2rem] overflow-hidden shadow-2xl" id="jersey-card">
                            <!-- Background gradient -->
                            <div class="absolute inset-0" id="jersey-bg" style="background: linear-gradient(135deg, {{ $currentColor }}, {{ $currentColor }}dd, {{ $currentColor }}bb);"></div>
                            ...
                            <div class="relative p-6 pt-8 pb-10">
                                ...
                                <div class="text-center mt-2">
                                    <div class="inline-block bg-black/20 backdrop-blur-sm rounded-lg px-4 py-1.5">
                                        <span class="text-white font-black text-lg tracking-wider uppercase" id="preview-jersey-name">{{ strtoupper(explode(' ', $member->name)[count(explode(' ', $member->name)) - 1]) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl p-4 border border-slate-100 shadow-lg -mt-4 mx-3 relative z-10">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl flex items-center justify-center" id="preview-avatar" style="background-color: {{ $currentColor }}15;">
                                    <span class="text-lg font-black" id="preview-initials" style="color: {{ $currentColor }};">{{ strtoupper(substr($member->name, 0, 1)) }}{{ strtoupper(substr(explode(' ', $member->name)[1] ?? '', 0, 1)) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="font-bold text-slate-900 truncate" id="preview-name">{{ $member->name }}</div>
                                    <div class="text-sm text-slate-500 truncate" id="preview-email">{{ $member->email }}</div>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100">
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold" id="preview-role-badge" style="background-color: {{ $currentColor }}15; color: {{ $currentColor }};">
                                    {{ $currentRole->name ?? 'Membre' }}
                                </span>
                                <span class="text-xs text-slate-400" id="preview-position">{{ $membership->pivot->position ?: '—' }}</span>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Preview Card - Staff (non-joueur) -->
                    <div id="preview-staff" class="{{ $isPlayer ? 'hidden' : '' }}">
                        <div class="bg-gradient-to-br from-slate-800 to-slate-900 rounded-[2rem] p-6 relative overflow-hidden shadow-2xl">
                            <!-- Glow effect -->
                            <div class="absolute top-0 right-0 w-40 h-40 rounded-full blur-3xl" id="staff-glow" style="background-color: {{ $currentColor }}30;"></div>
                            
                            <div class="relative">
                                <!-- Avatar -->
                                <div class="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-4" id="staff-avatar" style="background-color: {{ $currentColor }}20;">
                                    <span class="text-3xl font-black" id="staff-initials" style="color: {{ $currentColor }};">{{ strtoupper(substr($member->name, 0, 1)) }}{{ strtoupper(substr(explode(' ', $member->name)[1] ?? '', 0, 1)) }}</span>
                                </div>
                                
                                <!-- Name -->
                                <div class="text-center">
                                    <div class="text-xl font-black text-white mb-1" id="staff-name">{{ $member->name }}</div>
                                    <div class="text-slate-400 text-sm mb-4" id="staff-email">{{ $member->email }}</div>
                                    
                                    <!-- Role badge -->
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold" id="staff-role-badge" style="background-color: {{ $currentColor }}20; color: {{ $currentColor }};">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" id="staff-icon">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                        </svg>
                                        <span id="staff-role-text">{{ $currentRole->name ?? 'Membre' }}</span>
                                    </span>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    const roleConfig = {
        ...
        'owner': { jersey: false, position: false, license: false, birthdate: false, isPlayer: false }
    };
